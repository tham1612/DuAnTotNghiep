                ...
                <div class="chat-wrapper d-lg-flex gap-1 mx-n4 mt-n4 p-1">
                    ...
                    <div class="chat-leftsidebar">
                        ...
                        <ul class="nav nav-tabs nav-tabs-custom nav-success nav-justified" role="tablist">
                            <li class="nav-item">
                                <a style="font-size:20px" class="nav-link active" data-bs-toggle="tab" href="#chats"
                                    role="tab">
                                    Tin nhắn
                                </a>
                            </li>
                                                                            ...
                                                                            <div
                                                                                ...
                                                                                @if (!empty($user->image))
                                                                                    <img src="{{ asset('storage/' . $user->image) }}"
                                                                                        ...
                                                                                        alt="">
                                                                                @else
                                                                                    <div class="rounded-circle avatar-xs text-center"
                                                                                        style="background-color: #dff0fa; color: #black; width: 40px; height: 40px; display: flex; align-items: center; justify-content: center;">
                                                                                        {{ strtoupper(substr($user->name, 0, 1)) }}
                                                                                    </div>
                                                                                @endif

                                                                                <span id="status-span-{{ $user->id }}"
                                                                                    ...
                                                                                    function fetchLatestMessage() {
                                                                                        $.get(`/latest-message/${currentUserId}/${otherUserId}`, function(data) {
                                                                                            ...
                                                                                            if (data && data.message) {
                                                                                                ...
                                                                                                // Cập nhật tin nhắn vào phần tử có class .latest-message
                    ...
                    `);
                                                                                                ...
                                                                                                // Nếu không có tin nhắn, làm trống phần tử .latest-message
                                                                                                $(`.latest-message[data-user-id="{{ $user->id }}"]`).html('');
                                                                                            }
                                                                                        });
                                                                                    }
                                    ...
                                    <div class="position-relative" id="users-chat">
                                        @if (isset($receiverId) && $receiverId && isset($userss) && $userss)
                                                                ...
                                                                <div class="d-flex align-items-center">
                                                                    <div
                                                                        ...
                                                                        @if (!empty($userss->image))
                                                                            <img src="{{ asset('storage/' . $userss->image) }}"
                                                                                class="rounded-circle avatar-xs"
                                                                                alt="">
                                                                        @else
                                                                            <div class="rounded-circle avatar-xs text-center"
                                                                                style="background-color: #dff0fa; color: #black; width: 40px; height: 40px; display: flex; align-items: center; justify-content: center;">
                                                                                {{ strtoupper(substr($userss->name, 0, 1)) }}
                                                                            </div>
                                                                        @endif
                                            ...
                                            <div class="chat-conversation p-3 p-lg-4" id="message-box"
                                                ...
                                                <div class="messages-box justify-content-between" id="message-list">
                                                    @php
                                                        // Lấy ID của người dùng hiện tại
                                                        $currentUserId = Auth::id();

                                                        // Truy vấn đến bảng messages với điều kiện sender_id và receiver_id
                                                        $messages = \App\Models\Message::where(function ($query) use (
                                                            $currentUserId,
                                                            $receiverId,
                                                        ) {
                                                            $query
                                                    ...
                                                    @if ($messages->isNotEmpty())
                                                        @foreach ($messages as $message)
                                                            ...
                                                            @else
                                                                ...
                                                                <div class="d-flex mb-2" style="margin-left:10px;">
                                                                    <div class="bg-info-subtle d-flex justify-content-center align-items-center"
                                                                        style="width: 40px; height: 40px; margin-right: 10px; border-radius: 50%;margin-left:5px;">
                                                                        A
                                                                    </div>
                                                                    <div
                                            ...
                                            @else
                                                <div class="d-flex justify-content-center" style="margin-top:300px">
                                                    <p class="text-center" style="font-size: 20px">Hãy trò chuyện với ai
                                                        đó</p>
                                                </div>
                                        @endif
                                    </div>
        ...
        <div class="offcanvas-body profile-offcanvas p-0">
            ...
            @if (isset($receiverId) && $receiverId && isset($user) && $user)
                <div class="p-3 text-center">
                    @if (!empty($userss->image))
                        <img src="{{ asset('storage/' . $userss->image) }}"
                            class="avatar-lg img-thumbnail rounded-circle mx-auto profile-img" alt="">
                    @else
                        <div class="rounded-circle avatar-xs text-center"
                            style="background-color: #5f93de; color: #fff; width: 40px; height: 40px; display: flex; align-items: center; justify-content: center;">
                            {{ strtoupper(substr($userss->name, 0, 1)) }}
                        </div>
                    @endif
                    <div class="mt-3">
