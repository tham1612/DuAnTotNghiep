...
<form onsubmit="return submitUpdateDateTask(event, {{$task->id}})">
    ...
    <div>
        ...
        <input type="datetime-local" id="end_date_{{$task->id}}" name="end_date"
               value="{{$task->end_date}}"
               class="form-control border-0 my-2" onchange="updateReminderOptions2({{$task->id}})"/>
    </div>
    <div class="mt-2">
        <strong class="fs-16">Thiết lập nhắc nhở</strong>
        <select name="reminder_date" id="reminder_date_{{$task->id}}" class="form-select">
...
<script>
    function updateReminderOptions2(taskId) {
        var endDateInput = document.getElementById('end_date_' + taskId).value;
    ...
    var reminderSelect = document.getElementById('reminder_date_' + taskId);
    ...
    function submitUpdateDateTask(event, taskId) {
        ...
        formData.append('start_date', document.getElementById('start_date_' + taskId).value);
        formData.append('end_date', document.getElementById('end_date_' + taskId).value);
        formData.append('reminder_date', document.getElementById('reminder_date_' + taskId).value);
        formData.append('_method', 'PUT');  // Để giả lập method PUT với Laravel

        $.ajax({
            url: `/update-event/` + taskId,
